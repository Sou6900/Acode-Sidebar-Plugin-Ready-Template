...
let pluginJson = {};
try {
    pluginJson = JSON.parse(fs.readFileSync("plugin.json", "utf-8"));
} catch (e) {
    console.warn(`${color.yellow}⚠️ Warning: plugin.json not found or invalid.${color.reset}`);
}

const version = pluginJson.version || "1.0.0";
const zipName = `dist-v${version}.zip`;
...
async function bundle() {
  ...
  console.log(`${color.cyan}Building (Minified)...${color.reset}`);
  ...
  try {
    ...
    await build({
      ...
      format: "iife",
      minify: true,           // 1. Minification start
      legalComments: 'none',  // 2. no comment
      treeShaking: true,      // 3. (Optimization)
      sourcemap: false,
      loader: {
        '.png': 'dataurl',
        '.css': 'text'
    ...
    // 🔹 Core plugin files
    if (fs.existsSync("dist/main.js")) archive.file("dist/main.js", { name: "main.js" });
...
(async () => {
  ...
  if (WATCH) {
    ...
    // Note: recursive true is safer for nested files
